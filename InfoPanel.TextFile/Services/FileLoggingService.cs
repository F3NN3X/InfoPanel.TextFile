...
namespace InfoPanel.TextFile.Services
{
    ...
    public class FileLoggingService : IDisposable
    {
        ...
        public FileLoggingService(ConfigurationService configService)
        {
            ...
            _logFilePath = Path.Combine(logDirectory, "TextFile-debug.log");
        ...
        private void InitializeLogging()
        {
            try
            {
                if (_configService.IsDebugLoggingEnabled)
                {
                    ...
                    // Log session start
                    WriteToLog("INFO", "=== TextFile Debug Session Started ===");
        ...
        private void WriteToLog(string level, string message)
        {
            ...
            lock (_logLock)
            {
                try
                {
                    ...
                    Console.WriteLine($"[TextFile] {logEntry}");
                }
                catch (Exception ex)
                {
        ...
        public void Dispose()
        {
            ...
            try
            {
                if (_logWriter != null)
                {
                    WriteToLog("INFO", "=== TextFile Debug Session Ended ===");
                    _logWriter.Close();
                    _logWriter.Dispose();
                    _logWriter = null;
                }
